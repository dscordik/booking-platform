...
def create_service(db: Session, current_user: User, service: CreateServiceRequest):
    get_service = resources_repository.get_service_by_parametres(db, service.title, service.description,
service.duration_minutes, service.price)
    ...
    new_service = resources_repository.create_service(db, current_user.id,
